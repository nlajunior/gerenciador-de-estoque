...
class OrderView():
    ...
    def index(self, request):
        return self.view.index(request=request, model=Order, pagina='Order', headers=['Pedido', 'Valor Total', 'Quantidade', 'Status', 'Data de Criação', 'Quantidade', 'Ações'],
                               template_html='list-order.html')

    def form(self, request, id=None):
        return self.view.form(request, Order, self.formClass.getForm(), 'orders', 'order', 'Novo pedido', 'Pedidos', id)

    def delete(self, request, id):
        return self.view.delete(request, Order, 'orders', id)